...
exports.serialize = (sock, msg, store) => {
    ...
    m.body = m.type == "conversation"? msg.message.conversation : m.type == "extendedTextMessage"? msg.message.extendedTextMessage.text : m.type == "imageMessage"? msg.message.imageMessage.caption : m.type == "videoMessage"? msg.message.videoMessage.caption : m.type == "viewOnceMessage"? msg.message[getContentType(msg.message)]?.caption : m.type == "viewOnceMessageV2"? msg.message[getContentType(msg.message)]?.caption : m.type == "documentMessage"? msg.message.documentMessage?.caption : m.type == "documentWithCaptionMessage"? msg.message.documentMessage.caption : ""
    ...
    m.timeWib = moment().tz("America/Sao_Paulo").format("HH:mm:ss")
    ...
    m.reply = (teks, chatId = m.chat) => {
        if (m.replyType == "mess1") {
            sock.sendMessage(chatId, { text: teks, contextInfo: { mentionedJid: [...teks.matchAll(/@(\d{0,16})/g)].map((x) => x[1] + "@s.whatsapp.net"), forwardingScore: 999, isForwarded: true }}, { quoted: m })
        } else if (m.replyType == "mess2") {
            sock.sendMessage(chatId, { text: teks, contextInfo: { mentionedJid: [...teks.matchAll(/@(\d{0,16})/g)].map((x) => x[1] + "@s.whatsapp.net"), forwardingScore: 10, isForwarded: true }}, { quoted: m })
        } else if (m.replyType == "mess3") {
            sock.sendMessage(chatId, { text: teks, mentions: [...teks.matchAll(/@(\d{0,16})/g)].map((x) => x[1] + "@s.whatsapp.net") }, { quoted: m })
        }        
    }
    ...
    if (Object.keys(db.expired).includes(m.botNumber)) {
        for (let x of Object.keys(db.expired[m.botNumber].owner)) {
            if (Date.now() >= db.expired[m.botNumber].owner[x].expired) {
                ...
                m.reply("Seu tempo como proprietário acabou", x)
            }
        }
    }
//=================================================//
    if (Object.keys(db.expired).includes(m.botNumber)) {
        for (let x of Object.keys(db.expired[m.botNumber].premium)) {
            if (Date.now() >= db.expired[m.botNumber].premium[x].expired) {
                ...
                m.reply("Seu tempo premium acabou", x)
            }
        }
    }
//=================================================//
    if (!config.vipSewa.includes(m.chat) && Object.keys(db.chats).includes(m.chat) && db.chats[m.chat].sewa.status) {
        if (Object.keys(db.expired[m.botNumber].sewa).includes(m.chat)) {
            ...
            if (Date.now() >= db.chats[m.chat].sewa.expired) {
                ...
                db.chats[m.chat].sewa = { status: false, date: calender, expired: 0 }
                m.reply("O período VIP para este grupo expirou")
            }
        }
        ...
        for (let x of Object.keys(db.expired[m.botNumber].sewa)) {
            if (!config.vipSewa.includes(x) && Date.now() >= db.expired[m.botNumber].sewa[x].expired) {
                ...
                m.reply("O período VIP para este grupo expirou", x)
            }
        }
    }
//=================================================//
    if (m.autoLevel && !m.key.fromMe && !m.isPremium) {
        if (db.users[m.sender].xp == 3) {
            ...
            if (db.users[m.sender].level !== "Primordial Glory") {
                ...
                let teks = "\`\`\`「  LEVEL UP  」\`\`\`\n\n"
                teks += `▸ *Nome* : ${m.pushName}\n`
                teks += `▸ *Usuário* : @${m.sender.split("@")[0]}\n`
                teks += `▸ *Saldo* : + ${Number(limitNumber * 1000)}\n`
                teks += `▸ *Gemas* : + ${Number(limitNumber)}\n`
                teks += `▸ *Nivel* : ${levelRoleSebelum} => ${levelRole}\n`
                teks += `▸ *Horas* : ${m.timeWib}`
                m.reply(teks)
            }
