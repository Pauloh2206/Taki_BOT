...
const thumbnail = await getBuffer(ppimg)
return sock.sendMessage(chatId, { text: teks, contextInfo: { mentionedJid: [...teks.matchAll(/@(\d{0,16})/g)].map((x) => x[1] + "@s.whatsapp.net"), externalAdReply: { showAdAttribution: true, title: `Konochiwa ${senderNumber}`, body: "NO TOQUES AQUÍ", mediaType: 1, renderLargerThumbnail: true, thumbnail, sourceUrl: `https://wa.me/${senderNumber}` }}})
...
console.log(chalk.whiteBright("├"), chalk.keyword("blue")("[ ATUALIZAÇÃO DO GRUPO ]"), `${senderNumber} entrou no grupo ${groupName}`)
} else if (isWelcome && isRemove && !isMe && db.chats[from].setwelcome[anu.action] !== "") {
setReply(teksWelcome)
console.log(chalk.whiteBright("├"), chalk.keyword("blue")("[ ATUALIZAÇÃO DO GRUPO ]"), `${senderNumber} deixou o grupo ${groupName}`)
} else if (isWelcome && isPromote && !isMe && db.chats[from].setwelcome[anu.action] !== "") {
setReply(teksWelcome)
console.log(chalk.whiteBright("├"), chalk.keyword("blue")("[ ATUALIZAÇÃO DO GRUPO ]"), `${senderNumber} foi promovido`)
} else if (isWelcome && isDemote && !isMe && db.chats[from].setwelcome[anu.action] !== "") {
setReply(teksWelcome)
console.log(chalk.whiteBright("├"), chalk.keyword("blue")("[ ATUALIZAÇÃO DO GRUPO ]"), `${senderNumber} foi rebaixado`)
}
