...
module.exports = {
    ...
    callback: async ({ sock, m, groupName }) => {
        ...
        if (!Object.keys(db.expired[m.botNumber].sewa).includes(idGroup)) return m.reply("Ainda não aluguei, mano")
        let groupMetadata = await sock.groupMetadata(idGroup)
        delete db.expired[m.botNumber].sewa[idGroup]
        await m.reply("Sucesso ao deletar o aluguel " + groupMetadata.subject)
        ...
        if (!Object.keys(db.expired[m.botNumber].sewa).includes(m.chat)) return m.reply("Ainda não aluguei, mano")
        delete db.expired[m.botNumber].sewa[m.chat]
        await m.reply("Sucesso ao deletar o aluguel " + groupName)
        } else {
        if (listSewa.length == 0) return m.reply("Ainda está vazio irmão")
