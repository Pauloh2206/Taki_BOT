...
module.exports = {
    ...
    isVip: true,
    callback: async ({ m }) => {
        if (Object.keys(db.expired[m.botNumber].vip).length == 0 && Object.keys(db.expired[m.botNumber].owner).length == 0) return m.reply("Não há nada que possa ser limpo, mana")
