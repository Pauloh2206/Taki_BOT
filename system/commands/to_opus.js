...
module.exports = {
    ...
    callback: async ({ sock, m, isVideo, isQuotedVideo, isQuotedAudio }) => {
        ...
        if (isQuotedVideo && Object.keys(m.quoted.message[m.quoted.type]).includes("seconds") && m.quoted.message[m.quoted.type].seconds > 600 || isVideo && Object.keys(m.message[m.type]).includes("seconds") && m.message[m.type].seconds > 600) { return m.reply("o tamanho é muito grande 🙂") }
        ...
        if (m.quoted.message[m.quoted.type].ptt) return m.reply("Isso já é um áudio 😒")
