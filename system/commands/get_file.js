...
module.exports = {
    ...
    callback: async ({ sock, m }) => {
        ...
        if (m.text.includes("node_modules")) return m.reply("O tamanho é muito grande irmã 🙂")
        if (m.text.includes("connection")) return m.reply("O tamanho é muito grande irmã 🙂")
        if (!m.text.includes(".js") && !m.text.includes(".cjs") && !m.text.includes(".py") && !m.text.includes(".css") && !m.text.includes(".jpg") && !m.text.includes(".jpeg") && !m.text.includes(".png") && !m.text.includes(".mp3") && !m.text.includes(".mp4") && !m.text.includes(".webp") && !m.text.includes(".gif") && !m.text.includes(".html") && !m.text.includes(".replit") && !m.text.includes(".nix") && !m.text.includes(".md")) return m.reply("Não é um arquivo mana")
        if (!data.includes(m.text)) return m.reply("Arquivo não encontrado")                 
        setTimeout(() => {
        m.reply("Enviando arquivos.......")
        }, 2000)
        setTimeout(() => {
