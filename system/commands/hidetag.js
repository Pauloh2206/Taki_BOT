...
module.exports = {
    ...
    callback: async ({ sock, m, groupMembers, isImage, isVideo, isViewOnce, isDocument, isQuotedDocument, isQuotedViewOnce, isQuotedImage, isQuotedVideo }) => {
        ...
        if ((isQuotedVideo? m.quoted.message[m.type].seconds : m.message[m.type].seconds) > 600) return m.reply("irmã, e muito grande")
        ...
        if (isQuotedViewOnce && getContentType(m.quoted.message) == "videoMessage" && m.quoted.message["videoMessage"].seconds > 600 || getContentType(m.message) == "videoMessage" && m.message["videoMessage"].seconds > 600) { return m.reply("irmã, e muito grande") }
