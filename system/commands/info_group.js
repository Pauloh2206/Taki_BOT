...
module.exports = {
    ...
    callback: async ({ sock, m, isBotGroupAdmins, groupName, groupMembers, groupAdmins, groupOwner, groupMetadata }) => {
        let teks = "\`\`\`「  INFO GROUP  」\`\`\`\n\n"
        teks += ` *•* Nome : ${groupName}\n`
        teks += ` *•* Proprietário : ${groupOwner !== undefined ? "@" + groupOwner.split("@")[0] : "Tidak diketahui"}\n`
        teks += ` *•* Criação : ${moment(groupMetadata.creation * 1000).tz("Asia/Jakarta").format("DD/MM/YYYY HH:mm:ss")}\n`
        teks += ` *•* Total Admins : ${groupAdmins.length}\n`
        teks += ` *•* Total Membros : ${groupMembers.length}\n`
