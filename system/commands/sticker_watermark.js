...
module.exports = {
    commands: ["take","robar","swm"],
    cooldown: 13,
    minArgs: 1,
    expectedArgs: "<texto>",
    example: "{prefix}{command} Mr.Sun|Paulo.H",
    isSewa: true,
    isPremium: true, 
    isWait: true,
    isMedia: {
    ...
    callback: async ({ sock, m, isQuotedImage, isQuotedVideo, isImage, isVideo, isQuotedSticker }) => {
        ...
        if (isQuotedVideo && Object.keys(m.quoted.message[m.quoted.type]).includes("seconds") && m.quoted.message[m.quoted.type].seconds > 10 || isVideo && Object.keys(m.message[m.type]).includes("seconds") && m.message[m.type].seconds > 10) { return m.reply("Você só pode baixar vídeos de até 10 segundos, mana") }
        const id = fs.readdirSync("./temp").filter((x) => !x.includes("Zzzzzzzzzz@4.0.4")).length + 1
        const media = await sock.downloadAndSaveMediaMessage((isQuotedImage? m.quoted : m), "./temp/" + id)
