...
module.exports = {
    ...
    callback: async ({ sock, m }) => {
        if (!m.text.includes("https://github.com/")) return m.reply("Isso não é um link do github mana") 
        if (m.text.split("https://github.com")[1].split("/").filter((x) => x !== "").length !== 2) return m.reply("Ocorreu um erro, pode ser necessário inserir um nome conta/repo") 
        ...
        if (data.size > 70000) return m.reply("É realmente grande irmã, que grande 🙂") 
