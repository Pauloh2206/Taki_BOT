...
//        if (m.input.startsWith("08")) return m.reply("Gunakan code negara kak")
        ...
        return m.reply("O usuário já é premium")
        }
        if (Object.keys(db.database).includes(m.sender)) {
        if (db.database[m.sender].command !== "aggpremium") { db.database[m.sender].command = "aggpremium" }
        ...
        db.database[m.sender] = { command: "aggpremium", id: m.input, expired: "" }
        }        
        m.reply("\`\`\`「 ADICIONAR PREMIUM 」\`\`\`\n\n1 = PERMANENTE\n2 = AÑOS\n3 = MESES\n4 = SEMANAS\n5 = DIAS\n6 = HORAS\n7 = MINUTOS\n8 = SEGUNDOS")
    }
}
